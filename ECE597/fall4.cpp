...
void calibrate_thresholds(int &chest_acc_thresh_x, int &chest_acc_thresh_z, int &thigh_acc_thresh_x,
  int &thigh_acc_thresh_z, int &gyro_thresh_x_hi, int &gyro_thresh_x_lo, int &gyro_thresh_z_hi, int &gyro_thresh_z_lo)
{
  printf("Now configuring thresholds manually...\n");
  printf("Set Chest Accelerometer X Threshold: ");
  cin >> chest_acc_thresh_x;
  cout <<endl;
  printf("Set Chest Accelerometer Z Threshold: ");
  cin >> chest_acc_thresh_z;
  cout <<endl;
  printf("Set Thigh Accelerometer X Threshold: ");
  cin >> thigh_acc_thresh_x;
  cout <<endl;
  printf("Set Thigh Accelerometer Z Threshold: ");
  cin >> thigh_acc_thresh_z;
  cout <<endl;
  printf("Set Gyroscope X Threshold (High): ");
  cin >> gyro_thresh_x_hi;
  cout <<endl;
  printf("Set Gyroscope X Threshold (Low): ");
  cin >> gyro_thresh_x_lo;
  cout <<endl;
  printf("Set Gyroscope Z Threshold (High): ");
  cin >> gyro_thresh_z_hi;
  cout <<endl;
  printf("Set Gyroscope Z Threshold (Low): ");
...
void receive_data(int fd, uint8_t received_bytes[RECEIVED_BYTES_SIZE], int &current_cnt, int &time_to_fall, int &time_to_up_sit,
                  ...
                  int gyro_thresh_z_hi, int gyro_thresh_z_lo, int gyro_thresh_x_hi, int gyro_thresh_x_lo, uint8_t data_to_send[SENDING_DATA_SIZE],
...
void receive_data_cal(int fd, uint8_t received_bytes, int current_cnt)
{
  /*int max = 0;
  for (int j = 0; j < 20; j++)
  {
    ...
    //  printf("\n");
      ...
      /////////////////////////////////////////////////////////
       ...
       if(device_id == 0x57AA) // device from chest
       {
           ...
           {
             max = chest_acc_z;
             chest_acc_thresh_z = max;
             printf("New CHEST ACC Z: %d", chest_acc_thresh_z);
           }
       }
     }
   }
...
int main()
{
  ...
  int gyro_thresh_x_hi= 1100;
  int gyro_thresh_x_lo= 300;
  int gyro_thresh_z_hi = 1100;
  int gyro_thresh_z_lo = 300;
  ...
  while (mode != 3)
  {
     ...
     if (mode == 0)
     {
       printf("Now calibrating fall detectors. Please attach sensors to body\n");
       printf("Press enter to continue!\n");
       cin.ignore();

       printf("Lean forward for 2 seconds \n");
       //receive_data_cal(fd, received_bytes, current_cnt);
     }
